...
namespace Puzzle8
{
    public partial class IngresarConfiguMeta : Form
    {
        public static List<int> listaInicioConfigu = new List<int>();
        public IngresarConfiguMeta()
        {
            InitializeComponent();
            CenterToScreen();
        }
        public IngresarConfiguMeta(List<int> listaInicio)
        {
            InitializeComponent();
            CenterToScreen();
        ...
        static int coincidencias(List<int> listaMeta)
        {
            int cont = 0;
            for (int i = 0; i < 9; i++)
            {
                if (listaMeta[i] == listaInicioConfigu[i])
                {
                    cont++;
                }
        ...
        private void btnContinuar_Click(object sender, EventArgs e)
        {
            ...
            List<int> listaMeta = new List<int>{ 0, 0, 0, 0, 0, 0, 0, 0, 0};
            listaMeta = llenarListaMeta(listaMeta,textBox1, textBox2, textBox3, textBox4, textBox5, textBox6, textBox7, textBox8, textBox9);
           
            //Verificando que los números no se repitan.
            if (validacionDeConfigu(listaMeta) || coincidencias(listaMeta)==9)
            {
                if (coincidencias(listaMeta) == 9)
                {
                    MessageBox.Show("Las confuguraciones son iguales", "Error");
                    listaMeta.Clear();
                }
            ...
            else if(coincidencias(listaMeta) != 9)
            {
                modoInteligente FormModoInteligente = new modoInteligente(listaInicioConfigu, listaMeta);
                this.Visible = false;

                FormModoInteligente.Visible = true;
        ...
        {
            IngresarConfiguInicio ii = new IngresarConfiguInicio();
            this.Visible = false;
            ii.Visible = true;
        }
